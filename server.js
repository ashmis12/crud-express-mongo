...
MongoClient.connect('mongodb+srv://dbgod:8J0PMrEpRLh8IJoD@teamfacex.ddmcn.mongodb.net/scheduler?authSource=admin&replicaSet=atlas-2d1eur-shard-0&readPreference=primary&appname=MongoDB%20Compass&ssl=true', (err, database) => {
...
app.get('/initiate-session', async (req, res) => {
  const config = {
    method: 'post',
    ...
    data: {
      CustomerIdentifier: req.query.userId,
      CountryCode: "ARE"
    }
  };
  console.log(config);
...
app.get('/initiate-payment', async (req, res) => {
  const config = {
    ...
    data: { InvoiceAmount: 100, CurrencyIso: 'AED', CustomerIdentifier: req.query.userId }
  };
  console.log(config);
...
// 0bf0be13-6cea-4c2d-9a0b-c18ca975f397
